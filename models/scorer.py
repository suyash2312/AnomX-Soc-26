# models/scorer.py
#
# ForexGuardScorer — loads the trained Isolation Forest and scores events.
# Used by both the FastAPI endpoint and the Kafka consumer.
#
# Usage:
...
# This must match the column order in isolation_forest.py
FEATURE_COLS = [
    "hour_of_day", "day_of_week", "is_weekend",
    "login_success", "failed_attempts", "timezone_gap_hours",
    "lot_size", "trade_volume", "pnl", "margin_used",
    "trade_duration_seconds", "trade_volume_vs_baseline", "is_night_trade",
    ...
    "roll_5_trade_vol_mean", "roll_5_trade_vol_std", "roll_5_pnl_mean", "roll_5_margin_mean",
    "roll_10_trade_vol_mean", "roll_10_trade_vol_std", "roll_10_pnl_mean", "roll_10_margin_mean",
    "roll_5_click_rate_mean", "roll_5_session_dur_mean",
    "roll_10_click_rate_mean", "roll_10_session_dur_mean",
    ...
    "rolling_failed_attempts_5", "rolling_timezone_gap_5",
    "roll_5_deposit_sum", "withdrawal_to_deposit_ratio", "rolling_immediate_withdrawal_5",
    "deposit_amount_rolling_std", "deposit_count_last_10", "deposit_amount_rolling_mean",
    "withdrawal_amount_vs_account_age", "is_large_withdrawal",
    "trade_vol_zscore", "pnl_zscore", "amount_zscore",
    "session_duration_zscore", "click_rate_zscore", "trade_vol_vs_baseline_zscore",
]

# ── Feature name → human readable description ─────────────────────────────────
# Used to generate plain-English alert reasons
...
# ── Severity thresholds ───────────────────────────────────────────────────────
# Based on percentile distribution of training anomaly scores.
# Scores are the flipped IF decision_function output (higher = more anomalous).
# These thresholds were chosen by looking at the 70th, 85th, 95th percentile
# of anomalous event scores from the training set.
SEVERITY_THRESHOLDS = {
    "CRITICAL": 0.06,
    "HIGH":     0.03,
    "MEDIUM":   0.01,
...
class ForexGuardScorer:
    """
    Loads the trained Isolation Forest model and scores incoming events.
    Provides anomaly score, severity, plain-English reasons, and top features.
    """
    ...
    def _build_feature_vector(self, event: dict):
        """
        Extract features in the correct order from an event dict.
        Missing features default to 0.0.
        Returns a DataFrame with correct column names so sklearn
        does not raise feature name warnings during scaling.
        """
        import pandas as pd
    ...
    def _get_top_features(self, raw_vector: np.ndarray, scaled_vector: np.ndarray, top_n: int = 5):
        """
        Find the top N features with highest absolute scaled value.
        High absolute scaled value = feature is far from normal = likely contributing to the score.
        Returns list of dicts with feature name, raw value, and scaled value.
        """
        raw    = raw_vector.values.flatten()
        scaled = scaled_vector.flatten() if hasattr(scaled_vector, 'flatten') else scaled_vector.values.flatten()

        # rank by absolute scaled value
    ...
    def _get_reasons(self, top_features: list) -> list:
        """
        Convert top feature names to plain-English reasons.
        Returns up to 3 unique reasons.
        """
        reasons = []
    ...
    def score(self, event: dict) -> dict:
        ...
        Args:
            event: dict with feature values (can contain extra keys, they are ignored)

        Returns:
            dict with anomaly_score, is_anomaly, severity, verdict, reasons, top_features
        """
        if not self.loaded:
        ...
        # get score — flip sign so higher = more anomalous
